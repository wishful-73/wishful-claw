 ...
 *
 * Automatic compression status messages carry a display anchor and render inside
 * the active assistant reply. Manual compression status messages have no anchor
 * and render as standalone transcript rows. Both states update the same persisted
 * message in place.
 */
export function CompressionStatusMessage({
  ...
  if (status.state === 'compressed' && status.summaryText?.trim()) {
    const summaryMessage: UnifiedMessage = {
      id: status.summaryMessageId ?? `${message.id}:summary`,
      role: 'user',
      content: status.summaryText,
      createdAt: status.completedAt ?? message.createdAt,
      meta: {
        compactSummary: {
          messagesSummarized: status.messagesSummarized ?? 0,
          recentMessagesPreserved: true,
          ...(status.summarizerFailed ? { summarizerFailed: true } : {})
        }
      }
    }
    return <ContextCompressionMessage message={summaryMessage} />
  }
