...
状态：验证中，静态检查通过，等待用户 Windows/Electron 实测与最终裁定
日期：2026-08-29
分支：`dev/v2-iter-23`
...
- [x] 在 `src/main` 内实现最小内聚的窗口定位辅助逻辑，或在两个入口文件内以明确函数实现。定位输入分两类：
...
- [x] 用 Electron `screen.getAllDisplays()` 与 `Display.workArea`（x/y/width/height）获取显示器并计算居中、caret 下方/上方偏移和矩形 clamp。
- [x] bridge 上报的原生坐标统一标注为 screen-physical；进入 Electron 定位前通过窄类型适配器调用运行时 `screen.screenToDipRect(null, rect)` 转为 DIP，再与 `Display.workArea` / `BrowserWindow` bounds 运算。当前 Electron 43 类型文件未声明该运行时 API，适配器做了能力检测；API 缺失或失败时按目标显示器 `scaleFactor` 明确降级。Electron API 的 `screen`/`Display` bounds 与 work area 使用 DIP 坐标，未直接混用 Windows 物理像素。
- [x] 处理无效 HWND、无效/空 caret、显示器热插拔或矩形完全不可见时的回退；所有路径最终限制在目标 display work area 内。
- [x] 验证检查点：静态检查确认启动器与剪贴板使用各自回退顺序、所有新增位置计算最终经过 work area clamp；无主窗体聊天文件改动；`git diff --check` 和 node TS 配置通过。

### 步骤 2：快速搜索启动器多屏定位与 blur 生命周期

- [x] 修改 `src/main/quick-launcher.ts` 的创建与复用显示路径：快捷键唤起时选择当前焦点所在屏幕，初次创建和再次显示使用同一定位函数。
- [x] 保留 `forceActivateWindow()`、`show` 事件中的 reset 和已有 renderer focus 重试；不改动已有 Windows 激活桥的语义。
- [x] 为 blur-hide 增加窗口显示/激活后的短暂 grace period（命名常量 `LAUNCHER_ACTIVATION_GRACE_MS = 250`）；blur 触发时仅安排延迟确认，不立即隐藏。
- [x] 延迟确认区分两类情况：
  - 若窗口在确认前重新获得焦点，取消待隐藏；
  - 若确认时仍失焦且已离开显示激活 grace period，则隐藏；确认前仍处于 grace period 时继续等待一次，不把瞬时切焦误判为关闭。当前实现不额外查询鼠标位置或当前前台 HWND，真实外部点击/切焦行为列入 Windows 实测。
- [x] 在 `focus`/`show`/再次激活时清理待隐藏 timer；主动 Esc、启动应用、内部显式关闭保持立即关闭；外部点击保持关闭但允许短暂确认延迟。
- [x] 避免 timer 残留、窗口销毁后回调访问和重复 show/hide 竞态；`hide`/`close`/`will-quit` 路径统一清理 timer。
...
- [x] 修改 `src/main/priority-shortcuts.ts` 的 `ShortcutContext`、`BridgeMessage` 和内联 C# bridge 输出，形成明确可选契约：
...
- [x] 在主进程 JSON 解析处兼容新字段，并在进入 Electron 定位前将 screen-physical rect 用 `screen.screenToDipRect(null, rect)` 转成 DIP；当前 Electron 类型文件未声明该运行时 API，已通过窄类型能力检测并在 API 缺失/失败时按目标显示器 `scaleFactor` 回退，未静默混用两种坐标。
...
- [x] 修改 `src/main/clipboard-enhancer.ts`：快捷键回调接收 caret/焦点/前台窗口屏幕矩形及必要上下文，并在创建/复用窗口前计算目标 bounds。
- [x] 定位优先级：
...
- [x] 对每种定位结果统一执行 work area clamp；窗口不能超出目标显示器工作区；混合 DPI 下先完成 screen-physical → Electron DIP 转换再计算。
- [x] 保持现有 previous foreground/focus 保存、主动隐藏焦点恢复、blur 被动隐藏不恢复、粘贴顺序和 renderer 交互不变。
- 验证检查点：静态检查确认传统输入框 caret 优先、无 caret 时按焦点/前台/鼠标/主屏回退；剪贴板粘贴目标和焦点恢复路径未改动；三套 TypeScript 检查通过。真实输入框和多屏位置仍待 Windows/Electron 人工验收。

### 步骤 5：审查、完整验证与人工验收

- [x] 启动独立审查，检查范围是否越界、坐标系/工作区处理、timer 生命周期、bridge fallback、权限失败回退和是否误改主窗体；独立子任务超时后由主 agent 完成同清单静态复核，阻断项 0。
- [x] 运行静态验证：
  - `git diff --check`
  - `npx tsc --noEmit -p tsconfig.web.json`
  - `npx tsc --noEmit -p tsconfig.node.json`
  - `npx tsc --noEmit -p tsconfig.json`
- [ ] 如 docs/脚本要求，执行 `npm run build` 或对应 Electron build；`npm run build` 和 native bridge `Add-Type` 已通过，但真实 Electron/Windows 启动与快捷键冒烟尚未执行。
- [ ] 人工验证矩阵：
...
- [x] 产出本计划目录下 `review_report.md` 和 `verification_report.md`，记录工具输出、尚未完成的 Windows 实测项和未覆盖限制。
