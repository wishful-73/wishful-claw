...
将微信/飞书渠道和 Cron 定时任务从“基础链路已存在”打磨到可持续使用：会话标题稳定显示真实用户/群聊名称，渠道前缀只拼接一次；后台任务可以向指定微信/飞书会话主动发送消息；Cron 任务进入 SQLite，支持启动恢复、执行状态记录和 UI 管理闭环。
...
- [ ] 步骤1：统一渠道标题生成规则。将渠道显示名、群聊名、用户显示名、chatId 回退和前缀去重集中到一个可复用 helper；Main 创建/更新会话时只生成一次，renderer hydration 只使用 payload/DB 标题，不再次无条件拼接。兼容历史 `飞书:`/`微信:` 重复前缀，避免继续叠加。验证：新增私聊/群聊事件的标题断言覆盖名称优先级、chatId 回退和重复前缀输入；TypeScript web/node 零错误。
- [ ] 步骤2：修复渠道标题持久化和后续消息复用。确保同一 `pluginId + chatId` 路由到已有会话时不因新消息重复更新标题；当历史标题是占位标题或旧 ID 标题时才用最新名称替换。验证：连续发送两条飞书消息、两条微信消息，DB 标题保持单一前缀且名称稳定；已有用户自定义标题不被覆盖。
...
- [ ] 步骤5：设计并创建 Cron 数据表和迁移。新增 Cron 任务实体/Row/Mapper，字段覆盖任务配置（id/name/sessionId/scheduleJson/prompt/agentId/model/workingFolder/deliveryMode/deliveryTarget/pluginId/pluginChatId/deleteAfterRun/maxIterations/enabled/deletedAt）和任务级运行状态（lastFiredAt/lastRunAt/lastRunStatus/lastRunSummary/lastError/fireCount/createdAt/updatedAt）。在 `DbClient` 增加 `CREATE TABLE IF NOT EXISTS`、索引和旧库 `EnsureColumn` 迁移；补齐 AOT JsonContext。验证：新库和已有库初始化均成功；C# build 0 错误 0 警告。
...
- [ ] 步骤11：将 Automation 占位页替换为定时任务管理页。新增任务列表、状态筛选、启用/禁用、删除、运行一次、最近运行状态、下次执行时间和错误摘要；复用现有 UI store、IPC client 和组件规范。验证：从 WorkspaceSidebar 的 Automation 入口进入真实页面，列表能从 DB 加载并操作任务；TS web 零错误。
...
- [ ] 步骤13：保留日历预览并补执行反馈。按 Reasonix 风格组织列表/详情/日历预览，显示最近执行结果和渠道发送结果；订阅 Cron 事件实时更新当前任务，不依赖整页刷新。验证：任务触发时 UI 显示 fired/running/finished/error；页面切换回来状态仍从 DB 恢复。
