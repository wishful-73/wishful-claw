...
- 已执行：步骤 28 后续补齐（用户反馈“API Key 是必须的”）：新建服务商弹窗 API Key 占位提示去掉“可稍后在配置页补充”，添加按钮与提交校验改为名称 + Key 均必填；二次补齐：模型列表区 `flex-1 min-h-0` 占满剩余高度（去掉固定高度区间与外层整页滚动），滚动只发生在列表内部；三次补齐：模型列表展示排序——已启用模型置顶，组内保持原序（仅展示层）；TS 三配置 0 错误。
