...
**输出**：`docs/plans/iter-v2-{N}/plan-XXX/compliance_report.md`

**Git 操作**：
```bash
git add docs/plans/iter-v2-{N}/plan-XXX/compliance_report.md
...
一个 Plan 往往会验证多轮。历史教训：逐轮往 `compliance_report.md` 尾部**追加**新轮正文，却从不回写上一轮的状态、从不递增 `plan.md` 的版本行、也从不 commit —— 结果文档第一行永远停在「第 1 轮 FAIL ❌8」，**阻断项账面只增不减**，看不出任何收敛进展。以下五条用于根除该失效模式：

1. **台账是唯一的进度口径**：`compliance_report.md` 顶部必须有一个「收敛台账」表，逐条列出 ❌ 编号（❌1…❌N）与每一项在最新一轮的状态（`已关闭（附代码证据）` / `半修，计入未清` / `未修` / `本轮新出`），并给出一行趋势轨迹（如 `第1轮 ❌8 → 第2轮 ❌3（关8）→ 第3轮 ❌2（关3）→ 第4轮 ❌3（关1，新出2）`）。**新增一轮时必须回写该表**，不允许只往下追加正文。
2. **正文只保留最新一轮**：报告正文只写本轮验证结论与新发现的 ❌/⚠️；上一轮 ❌ 的复核以台账里的一行状态 + 本节第 4 条要求的代码证据呈现。旧轮全文不进报告——需要追溯时从 git 历史取（每轮都要求 commit，见第 3 条），**不要用文档当历史**。经验体量口径：单轮报告正文控制在 150 行内，超过即说明有旧轮正文未被清除。
3. **每轮必须 commit**：本轮报告落盘 + `plan.md` 版本行递增后立刻 commit。未 commit 的验证轮次视为未发生（也无法被追溯，等于把收敛证据丢掉）。
4. **「已修」≠「已关闭」**：修订 ❌ 只能把状态写成 `已修待复核`，**必须由下一轮独立验证确认后才能改判 `已关闭`**，且关闭理由要指向验证者亲自打开过的 `file:line`（文档自述不作证据）。`plan.md` 顶部版本行必须与本台账同步：写清本版号、本版关闭了哪几项、哪几项仍是 `已修待复核`，并明确「**未经下一轮复核不得进入执行态**」。
5. **检查项一致性复核口径**：每轮验证除检查新增内容外，必须重跑两条机械检查——① `plan.md`「涉及文件」与本轮各步骤实际要动的文件集合是否一致（漏列文件即 ❌，因为执行期会照着清单改）；② `[自动]` 门禁是否**可直接运行**且口径互不冲突（同一命令不能一处要求 `-o`、另一处要求 0 warning）。这两条是上一轮 ❌13 / ⚠️29 这类问题的来源。
