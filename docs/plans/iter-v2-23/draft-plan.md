...
- 已执行：步骤 31 补齐：左侧“对话”分区改为与项目行同款可收起/展开（点击分区行切换全局会话列表，默认展开，子列表缩进/加载更多逻辑与项目子列表一致，chevron 旋转指示）；后按用户反馈移除分区行右侧会话计数；步骤 30 补齐：文件树组件内部图标同步上色并对齐 tab 色彩方向（目录图标统一 `sky-500/dark:sky-400` 覆盖树节点/搜索结果/新建输入/头部，`EXT_ICONS` 按状态色惯例改双模式静态类名表，兼容 Tailwind v4 扫描与 6 套主题明暗）；TS 三配置 0 错误。
