...
**仅 startup 模式使用启动检查**：维护"上次整理时间"水位（写配置文件或 DB）；配置为 `startup` 时，启动后按水位和节流窗口决定是否触发。配置为 `nightly` 时，正常启动不因水位过期补跑，只等待配置的本地时间点。
...
- `nightly` 模式：每 5 分钟检测跨过配置时刻 → 通知渲染端执行 S7；正常启动不因水位非今日而补跑，只等待配置的本地时间点
