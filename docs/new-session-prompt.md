...
3. `docs/iteration-plan.md` — 总体迭代计划（含 v2-iter-26 已完结与 v2-iter-27 后续路线）
...
| v2-iter-26 | 桌面自动更新体验收口 + 4 项 Obsidian 待办 + 项目变更面板 — Release Notes 安全富文本渲染、下载与弹窗生命周期解耦（状态横幅/托盘详情/安装可计数闸门/启动即确认）、字节级下载可观测与原生日志、渠道会话工具可见性归一 global、飞书扫码自动启用启动、内置服务商预设同步 17 个、OpenCode Go 注入 x-opencode-session、项目变更面板（Git 变更/Diff 预览/git 提交区）、服务商配置/模型双页签 | ✅ 已完成，产品版本 0.2.26，tag v0.2.26，已合并 main（更新板块真机升级验证移交 iter-27） |
...
- **v2-iter-26 已完结**（2026-09-09 老大确认按标准流程收尾发版）：已合并 main、tag `v0.2.26`、产品版本 `0.2.26`，Release 与安装包已发布
- 最新 tag：`v0.2.26`；新会话直接从 main 开始
- 交付内容：桌面自动更新体验收口（Release Notes 安全富文本、下载与弹窗生命周期解耦、托盘可恢复、安装可计数闸门、字节级下载可观测）、渠道会话工具可见性归一 global、飞书扫码绑定后自动启用并启动、内置服务商预设同步 17 个、OpenCode Go 请求注入会话头、项目变更面板（Git 变更/Diff 预览/git 提交区）、服务商配置/模型双页签。详见 `docs/PROGRESS.md` 的 v2-iter-26 段落
- **iter-26 移交 iter-27**：更新板块真机升级验证（用低于当前 Release 的安装版实际走 electron-updater 检查更新 → 下载确认 → 安装重启全流程）在 iter-27 发布时执行；渠道长期无人值守权限边界、并发串行化等 iter-25 遗留边界仍待后续处理
- 正式产品版（1.0）仍延后：正式版发布前各迭代收尾仍按标准流程打 0.2.x 版本与 tag

## 后续路线

1. **v2-iter-27：范围待与老大确认** — 首要候选：更新板块真机升级验证（随发版执行，用低于当前 Release 的安装版走完整 electron-updater 流程）；其余候选：渠道权限/并发等遗留、`docs/tool-slimming-analysis.md` 工具精简排期、Obsidian 知识库最新待办；开工前先与老大确认具体范围与验收标准
...
1. `git checkout main` + `git pull origin main`（失败时走代理）+ `git log --oneline -5` — 确认 main 已含 v0.2.26
2. 读 `AGENTS.md` — 查看 7 层架构和分层约定、分支管理规则（新分支必须从最新 main 拆出）
3. 读 `docs/PROGRESS.md` + `docs/iteration-plan.md` — 查看已完结迭代（最新 v0.2.26）与 v2-iter-27 路线；开工前必须与老大确认 v2-iter-27 具体范围、优先级与验收标准，不得按计划文档默认开工
4. 检查 `D:\koda\Obsidian\02-AI教学\wishfulclaw` 中最新的 Bug 和优化建议（若路径不存在则跳过并告知老大）
5. 确认后 `git checkout -b dev/v2-iter-27` 开工；不得在 main 上直接开发，不得提前发布正式版（1.0）
