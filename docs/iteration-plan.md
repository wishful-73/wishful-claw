...
### v2-iter-24：任务面板（进行中，范围已确认）

**目标**：落地任务面板——全局任务（完整字段：标题/状态/标签/优先级/截止时间/描述/关联会话·项目；入口为左侧扩展区 Task Board；agent 经全局任务工具自主维护）+ 会话级任务（参考 OpenCowork：agent 自行创建维护的临时任务队列，独立 `session_tasks` 表 + TaskCreate/Get/Update/List 工具 + 聊天流卡片展示）。两套任务均为全新独立表，与 Goal 模式表系无任何关系。

**边界**：本迭代不做任务“一键执行”（自动开会话跑任务），仅回写状态；会话级任务不面向用户手动创建。
...
v2-iter-1 ~ v2-iter-23 已完成；v2-iter-24（任务面板）当前执行中。
