...
> **修订（2026-09-19 / iter-33 S-95）**：压缩改为**滚动摘要**后，上式三处语义收紧：
> - `[pinned prefix]` 只含 system + 首条 user turn，**不再包含历史摘要** —— 旧摘要此前被 pin 进 head，正是它导致摘要 1 → 2 → … → 53 单调累积；
> - `[compaction summary message]` 在**正常态只有一条**（本次新生成的）；旧摘要的内容已折进它，随其一起移除；
> - **摘要生成失败时**回退为 `[pinned prefix] + [旧摘要 + 保留的 user turns] + [机械摘要] + [recent tail]` —— 机械摘要零信息量，此时必须保留旧摘要，否则模型视角会丢掉更早的历史。
>
> 字段与顺序契约不变，变的是「哪些消息进入结果」（只改模型看到的 wire，DB 与聊天记录一字不删）。
...
> **修订（2026-09-19 / iter-33 S-95）**：判据由**条数**改为 **token 估算**（`EstimateMessagesTokens(新) < EstimateMessagesTokens(旧)` 才算压动）。条数会骗人 —— 一条长摘要换掉两条短消息，条数减少但 token 净增，旧判据把这种情况放行成了 `compressed`，正是「越压越多」的放行口。`newCount`/`originalCount` 继续作为状态卡展示字段保留，但不再充当成功判据。
