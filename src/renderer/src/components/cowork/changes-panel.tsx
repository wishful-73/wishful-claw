...
export function ChangesPanel({ workingFolder }: { workingFolder: string }): React.JSX.Element {
  ...
  return <div className="agent-files-explorer flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      ...
      {changeRows.map(({ file, section }) => <button key={`${section}:${file.path}`} type="button" onClick={() => setSelected({ repo, file, staged: section === 'staged' })} className={cn('group flex w-full items-center gap-2 rounded-none border-0 bg-transparent px-3 py-1.5 text-left text-xs text-agent-files-fg transition-colors hover:bg-agent-files-hover focus-visible:bg-agent-files-hover', selected?.file.path === file.path && 'bg-agent-files-hover')}><span className="font-mono text-muted-foreground">{section === 'untracked' ? 'U' : section === 'conflicted' ? '!' : section === 'staged' ? file.stagedStatus : file.unstagedStatus}</span>{section === 'untracked' ? <FilePlus className="size-3.5" /> : <File className="size-3.5" />}<span className="truncate font-mono">{file.path}</span></button>)}
