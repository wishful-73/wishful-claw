...
| 节点 | 必须停 | 原因 |
|------|--------|------|
| **讨论态 → 实施态切换** | ✅ | 老大明确拍板方案（"改吧 / 动手 / 按这个推进"）才开工；他补充口径、报问题不算拍板。见 agents.md「协作纪律（硬规则）」 |
...
> **硬规则：任何一份 md 都不得超 500 行 / 60 KB，超了必须拆。**
> 依据是实测：iter-33 的 `raw-requirements.md` 长到 **187 KB / 1341 行**、`compliance_report.md` **168 KB**，iter-34 的 `raw-requirements.md` **140 KB**。单文件一胖，插入、搬移、检索全变成体力活 —— 改一处要通读全文，白付大量上下文，还容易改出「只改数字又造出另一个错」这类伤。

**一律采用「总览清单 + 明细子文档」两层：总览只放索引与状态，正文一律落在明细子文档里。**
...
```
docs/
├── iteration-plan.md            # 迭代总览（≤150 行）：拆分规则 / 完结规则 / 迭代索引表 / 依赖关系
└── iterations/
    └── {v1|v2}-iter-{NN}.md     # 单迭代明细：目标 / 需求 / 步骤 / 验证标准
```

**单个迭代：**

```
docs/plans/iter-v2-{N}/
├── plan.md                  # 迭代总览（≤150 行）：目标 / 需求索引 / 刀序 / 门禁基线
├── raw-requirements.md      # 需求清单（≤150 行）：一行一需求，链到 requirements/
├── requirements/
│   └── S-{NNN}.md           # 单需求明细：原文 / 勘测 / 裁定 / 实施 / 验收修正
├── plan-s{NNN}/             # 该需求单开的计划与报告（复杂需求才开）
│   ├── plan.md
│   ├── compliance_report.md
│   └── verification_report.md
└── changelog.md             # 裁定与口径变更流水（按日期追加，跨需求）
```
...
| 文档 | 装什么 | 不装什么 |
|---|---|---|
| `raw-requirements.md` | 需求清单表格（编号 / 标题 / 状态 / 明细链接）+ 文末候选池 | 需求正文、勘测过程、实施细节 |
| `requirements/S-{NNN}.md` | **只装这一个需求**：原文、勘测、裁定、实施记录、验收修正 | 别的需求的内容、跨需求的裁定流水 |
| `changelog.md` | 按日期追加的裁定 / 口径变更流水 | 需求正文 |

**为什么流水单独一个文件**：裁定天然跨需求（一条流水可能同时改两个需求的方案），塞进任何单需求文档都会打架。iter-34 就栽过 —— 「裁定记录」夹在 S-114 与 S-115 中间，读的人以为 S-115 起全是它的内容。

### 落笔纪律

- 登记新需求 = **清单加一行 + 新建一个明细文件**，绝不往清单里贴正文
- 需求明细膨胀到 500 行 ⇒ 就地再拆（按「原文 / 实施 / 验收」分文件），不靠"忍"
- 前面各阶段写的「输出：`docs/plans/plan_XXX/xxx.md`」，路径按本节约定解释
- 一份文档只写一个主体；跨主体的内容宁可多开一个文件
...
- **文档规模按「文档组织」节执行**——任何 md 不得超 500 行 / 60 KB，超了就地拆；登记需求只动总览清单那一行，正文进 `requirements/S-{NNN}.md`，跨需求的裁定流水进 `changelog.md`
