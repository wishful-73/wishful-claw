...
- 状态：已完成，已合并 main（2026-09-09 收尾发版，tag `v0.2.26`）
- 分支：`dev/v2-iter-26`
...
- [✓] B2：改造 Main 下载启动语义。`update:download` 在 `downloadUpdate()` 成功启动后立即返回 `{ success: true, operationId }`，不等待完整下载；后台 promise 必须自行捕获失败并广播，关闭弹窗/隐藏窗口不触发取消。Mini 验证：`npm run test:updater-state` 断言“启动确认先于完成事件”、重复请求不启动第二个下载、close/hide 不产生取消或安装动作，再执行统一 Mini 门槛。（已完成：新增 `createUpdateDownloadGate` single-flight 闸门，`requestUpdateDownload` 返回 `UpdateDownloadStartResult` 且不再 await 原生 promise，失败经 `onFailure → setError` 广播；`downloadPromise` 替换为 `downloadGate.isInFlight()`；47 项断言通过，含 ack 先于 native-completed、三次重复请求仅 1 次 `start()`、后台失败后可重试并 mint 新 operationId、旧 operation 的迟到完成事件被拒；close/hide/quit 不触达更新链路经源码核验：`src/main/index.ts:100-105` 仅 `hide()`、`:147-153` 托盘菜单只有显示/退出、`:567-581` `before-quit` 无 updater 调用；三套 `tsc --noEmit -p` 0 错误、`dotnet build` 0 警告 0 错误、`npm run build` 成功、`git diff --check` 干净）
- [ ] B3：实现顶层横幅、弹窗恢复与托盘“更新详情”。托盘动作执行 restore/show/focus 后发送 `update:show-details`；Renderer 收到后打开详情并以 `update:status` 快照为准。托盘“退出”只设置退出标志并 `app.quit()`。Mini 验证：开发态手动覆盖关闭弹窗、关闭主窗口、托盘恢复、Renderer reload 四条路径；状态与进度不丢，退出不调用安装；截图保存为 `docs/plans/iter-v2-26/evidence/background-download.png`，再执行统一 Mini 门槛。（**代码已完成**：新增 `src/renderer/src/components/updater/UpdateStatusBanner.tsx`，在 downloading/downloaded/error 三态常驻、刻意不提供关闭按钮（可关闭就等于把状态重新藏起来），downloaded 态直接给出“重启安装”；`src/main/index.ts` 抽出统一 `showMainWindow()`（restore/show/focus，托盘点击、“显示主窗口”与 second-instance 共用）与 `showUpdateDetails()`，托盘固定新增“更新详情”，先恢复窗口再发 `update:show-details`，“退出”仍只是 `isQuiting = true; app.quit()`；`src/shared/updater/types.ts` 新增 `UpdateShowDetailsPayload = null`，事件不携带状态，避免托盘持有第二份会过期的快照；`use-app-updater.ts` 抽出 `refreshStatus()` 供挂载与托盘共用，`downloadUpdate()` 改为返回 Main 是否接受启动；`App.tsx` 顶层挂载横幅、订阅 `update:show-details`（先刷新快照再打开弹窗，绝不展示过期 phase）、Main 确认启动后收起弹窗；zh/en `settings.json` 各补 `updater.banner.*` 五个键，两个文件均带 BOM，只用编辑器改不重写。`UpdateDialog.tsx` 本步不动——按钮改“后台下载”与 downloaded 态“重启安装/稍后”属于 B5 的四态视觉与文案。**统一 Mini 门槛已过**：三套 `tsc --noEmit -p`（web/node/根）0 错误、`dotnet build src/runtime/WishfulClaw.sln` 0 警告 0 错误、`npm run build` 成功、`test:updater-state` 47 项通过、`test:updater-release-notes` 71 项未回归、两个 locale JSON 解析通过、`git diff --check` 干净。**四条路径实机取证与 `background-download.png` 与 C4 合并执行**：关闭弹窗、关闭主窗口隐藏到托盘、托盘恢复、Renderer reload 都需要一次能真正触发 `update-available` 并进入 downloading 的开发态会话，而 C4 已为此临时下调 `package.json` 版本，共用同一次取证会话，避免重复启动与重复降版；不以走查或伪造截图冒充。其中“退出不调用安装”已静态核验：托盘“退出”仅置标志并 `app.quit()`，`src/main/index.ts` 的 `before-quit` 与 `window-all-closed` 均无 updater 调用，`quitAndInstall` 唯一调用点仍在 `requestUpdateInstall()`。）
- [✓] B4：收口安装硬契约。Main 的 `requestUpdateInstall()` 保持唯一 `quitAndInstall(false, true)` 调用点；仅 `UpdateDialog` 与横幅中的“重启安装”按钮可调用 `installUpdate()`；重复点击幂等。`update-downloaded` 只广播并进入 downloaded，不调用安装。Mini 验证：`npm run test:updater-state` 断言下载完成、关闭/隐藏、托盘、重载、正常退出均产生 0 次安装调用，单次用户点击产生 1 次，重复点击仍为 1 次；安装调用同步抛错恢复 error，再执行统一 Mini 门槛。（已完成：新增 `createUpdateInstallGate`，把“能否安装”的判定收进可注入的闸门，硬契约由文档变成可计数的断言；`quitAndInstall(false, true)` 移入闸门的 `install` 闭包，`grep -rn quitAndInstall src/` 全文仅 `src/main/updater.ts:75` 一处真实调用（另两处为注释）；`requestUpdateInstall()` 改为消费三态结果——`refused` 返回失败、`started` 记日志、`already-installing` 静默返回成功，重复点击不再排第二次重启，100ms 延迟保留以确保 IPC 回包先于进程拆除；渲染侧 `installUpdate()` 唯一调用点是 `App.tsx` 传给 `UpdateDialog` 与 `UpdateStatusBanner` 的 `onInstall`，两者的“重启安装”按钮之外无任何入口，弹窗按钮 `disabled={isInstalling}`、横幅在 installing 态不渲染；`update-downloaded` 仍只 `applyDownloaded` + 广播。新增 9 项断言（共 56 项通过）：到达 downloaded 0 次安装、读快照（对应 reload/托盘“更新详情”/重开弹窗）0 次且两次快照 deepEqual、单次点击 1 次、三次点击仍 1 次、idle/available/downloading/失败检查四态全部 `refused` 且 0 次、下载失败保留 expectedVersion 但 0 次、版本不匹配的完成事件 0 次、`install` 同步抛错经 onFailure 恢复 error、失败后可重试并再次安装 1 次。关闭/隐藏与正常退出这两条路径不触达任何 updater 代码，已由 B2 的源码核验覆盖（`src/main/index.ts` 的 close 仅 `hide()`、`before-quit`/`window-all-closed` 无 updater 调用），不以伪造断言冒充。三套 `tsc --noEmit -p` 0 错误、`dotnet build` 0 警告 0 错误、`git diff --check` 干净。）
- [ ] B5：补齐中英文文案和 available/downloading/downloaded/error 四态视觉。下载中显示后台继续说明；downloaded 横幅持续提供恢复入口；error 保留重试。Mini 验证：开发态逐态截图与键盘操作检查；待安装截图保存为 `docs/plans/iter-v2-26/evidence/downloaded-awaiting-install.png`，再执行统一 Mini 门槛。（**代码已完成**：`UpdateDialog.tsx` 按四态重写页脚——available 显示“后台下载”（原“下载更新”，标签本身就要说明下载不占住界面）、downloading 显示“打开发布页 + 稍后”并在进度条下补两行说明（后台继续 / 托盘可恢复，分两行是为了中英文都不需要额外分隔符）、downloaded 与 installing 严格只留“稍后 + 立即重启安装”两个动作；error 态标题改为“更新失败”、原始错误信息留在下方 destructive 提示框，不再把裸错误串塞进 DialogDescription。**顺带修掉一个死路**：error 可由检查失败、下载失败、安装失败三条路径到达，原先一律按 `availableVersion` 选按钮，安装失败时（`downloadedVersion` 已在盘上）会显示“下载更新”，点下去 `requestUpdateDownload` 因 `downloadedVersion` 已存在而立即返回成功、既不重下也不安装；改为按 `downloadedVersion === expectedVersion` → “重试安装”、否则按 `expectedVersion` → “重试”（重下）、再否则 → “重试”（重查）三分。`percent` 为 null 时显示“未知”而不是 0%（0% 与卡死无法区分），`aria-valuenow` 同步省略为不确定态；这是本步的最小就地修法，C1 建立 `update-progress.ts` 后由其统一接管字节/速度/耗时。文案：zh/en `settings.json` 的 `updater.dialog` 各补 7 个键并把 `download` 改名，脚本核验 25 个被引用键在两种语言全部存在、两侧键集完全一致、无未被引用的死文案。横幅的 downloaded 恢复入口已在 B3 落地。**统一 Mini 门槛已过**：三套 `tsc --noEmit -p` 0 错误、`dotnet build` 0 警告 0 错误、`npm run build` 成功、`test:updater-state` 56 项、`test:updater-release-notes` 71 项未回归、`git diff --check` 干净。**逐态截图、键盘操作检查与 `downloaded-awaiting-install.png` 与 C4 合并执行**：四态都要一次能真正触发 `update-available` 的开发态会话才能到达，C4 已为此临时下调 `package.json` 版本，共用同一次取证会话；不以走查或伪造截图冒充。）
...
- [✓] C1：实现字节级状态、结构化日志和格式化展示。进度按 operationId 关联且不倒退；total/速度未知时显示“未知”而非 0；完成日志保留最后观测值与声明包大小。Mini 验证：`npm run test:updater-progress` 覆盖 unknown、0 字节、单调性、重复/旧事件、elapsedMs 与格式化边界，再执行统一 Mini 门槛。（已完成：**声明包大小落地**——`ProgressInfo` 不含安装包大小，只能从 `update-available` 的元数据取，新增 `resolveDeclaredInstallerSize(info)` 按 `info.path` 匹配 `info.files`（找不到则退回首项），仅在有限且 > 0 时采用，否则记 null；`UpdateAvailableInput` 增 `declaredInstallerSize`，`applyAvailable` 存入，`beginDownload` 不再清空（它描述的是“这次发布”而非“这次尝试”，重试必须保留基线）。**结构化日志**——`updater-state.ts` 导出 `pickProgressSnapshot` 与 `observeDownload`，后者在六个观测字段外补 operationId/currentVersion/expectedVersion/downloadedVersion，正是 C2 关联原生 `[updater]` 输出所需的键；`updater.ts` 新增 `logDownload`，start/progress/complete/fail 四类日志共用同一字段集，`setError` 增可选 `extra` 并在下载失败与 error 事件里附上**失败前**快照（`setError` 会把 phase 推到 error，先读才能留住最后观测值）。progress 日志按 5s 节流（`PROGRESS_LOG_INTERVAL_MS`），下载启动时把 `lastProgressLogAt` 归零，避免自有日志淹没原生 `Full`/`To download` 证据，同时保证每次尝试的首条观测必然落盘。**三处数值同源**——`download-progress` 与 `update-downloaded` 的 IPC 载荷改为 `pickProgressSnapshot(snapshot)`，且**在协调器 apply 之后**读取，因此 UI、任务栏、日志、报告引用的是同一组经过钳制与单调校验的数；`UpdateDownloadProgressPayload` 由 `{percent}` 拓宽为整个快照，`UpdateDownloadedPayload` 继承快照再加 version（§3.2 固定的是通道名，不是载荷形状）。删掉了 `updater.ts` 与 `use-app-updater.ts` 里两处 `Math.max(0, Math.min(100, …))` 死钳制（协调器已做），任务栏在 percent 为 null 时改用 indeterminate 而非 0%。**格式化**——新增纯模块 `src/renderer/src/components/updater/update-progress.ts`，`createUpdateProgressFormatter(unknownLabel)` 一次绑定“未知”文案，返回 percent/bytes/speed/elapsed/transferredOfTotal；二进制单位（B/KiB/MiB/GiB/TiB），字节数**截断而非四舍五入**（1048575 四舍五入会打印成不存在的“1024 KiB”），真实 0 渲染为 `0 B`/`0 B/s`/`0%`/`0:00` 与未知严格可区分，NaN/Infinity/负数一律未知，transferredOfTotal 两侧各自独立渲染（一侧未知不影响另一侧）。`UpdateDialog` 用 `formatter.percent` 接管 B5 的临时“未知”写法，进度条下增一行“已下载/总量 · 速度 · 已用”，downloaded 态增一行去掉速度的“已下载/总量 · 已用”（已完成时显示速率会被误读为仍在传输）；`UpdateStatusBanner` 同步展示，弹窗关闭时后台下载也有真实数字，error 态该行让位给错误信息。文案：`updater.dialog.percentUnknown` 移入新的 `updater.progress.unknown`（横幅也要用），新增 `updater.progress.stats` 与 `updater.progress.completed`。**刻意不显示“差分/完整”标签**——按 Plan 判定规则该结论只能由原生日志得出，UI 上猜测即是伪造。测试：新增 `tests/updater-state/download-progress.ts` 与 `test:updater-progress`（28 项通过），覆盖 unknown、部分已知、0 字节、单调性（倒退被拒/相等接受/负数忽略/超 100 钳制）、旧 operation 与完成后事件被拒、elapsedMs 起跑为 0 且完成冻结、声明包大小 0 与缺失均记 null、完成保留最后观测值、快照与观测字段集精确断言、单位边界与 TiB 封顶、非有限值、unknown 标签可注入；`tests/updater-state/program.ts` 的本地 `numericSnapshot` 副本删除改用导出的 `pickProgressSnapshot`（56 项未回归）。**统一 Mini 门槛已过**：三套 `tsc --noEmit -p` 0 错误、`dotnet build` 0 警告 0 错误、`npm run build` 成功、`test:updater-release-notes` 71 项未回归、脚本核验 27 个 `updater.*` 被引用键在 zh/en 全部存在且两侧键集一致、无死文案、`git diff --check` 干净。本步无需实机取证，不与 C4 合并。）
- [✓] C2：保留并核验 electron-updater 原生日志。每次下载的自有日志以 operationId、currentVersion、expectedVersion 关联原生 `[updater]` 输出；报告按上述规则判定 differential/full/unknown。Mini 验证：模拟 progress/complete/error 序列后检查结构化字段完整；源码断言 `autoDownload=false`、`autoInstallOnAppQuit=false` 且未设置禁用差分的选项，再执行统一 Mini 门槛。（已完成：**原生日志保留已核验并锁定证据来源**——`configureUpdater` 把 `instance.logger` 的 info/warn/error/debug 四级全部转发到自有日志（info/debug → `logInfo`，warn → `logWarn`，error → `logError`），原生输出统一带 `[updater]` 前缀（debug 为 `[updater:debug]`）可直接 grep。已在本机安装的 electron-updater 6.6.2 中定位到判定规则所依赖的两条原生日志，二者恰好走**不同**级别，因此四级全转发不是冗余：`node_modules/electron-updater/out/differentialDownloader/DifferentialDownloader.js:58` 经 `logger.info` 输出 `` `Full: ${formatBytes(newSize)}, To download: ${formatBytes(downloadSize)} (${pct}%)` ``（差分证据），`node_modules/electron-updater/out/NsisUpdater.js:170` 经 `logger.error` 输出 `` `Cannot download differentially, fallback to full download: ${e.stack || e}` ``（完整回退证据）；另有 `downloadPlanBuilder.js:69` 的 `File … has N changed blocks` 可作辅助佐证。C5 报告即按这两条字符串判定。**关联键已由 C1 落地**：`observeDownload` 在六个观测字段外固定携带 operationId/currentVersion/expectedVersion/downloadedVersion，start/progress/complete/fail 四类自有日志共用该字段集，因此每条自有日志都能与同时间窗内的原生 `[updater]` 输出对齐。**新增 7 项断言（`test:updater-progress` 28 → 35 项通过）**：①模拟 start→progress×2→complete 序列，逐阶段断言观测记录字段完整且关联键正确、完成时保留最后观测值与声明包大小；②模拟 start→progress→error 序列，断言失败前读取（与生产错误路径一致）保住 operationId/expectedVersion 与最后字节数，`fail` 同样冻结 elapsedMs；③全程无任何测量时字段仍完整，percent 为 100 而其余为 null；④⑤⑥源码断言 `instance.autoDownload = false`、`instance.autoInstallOnAppQuit = false`、全文不出现 `disableDifferentialDownload`（禁用差分会让此后每次测量都变成完整下载，静默作废判定规则）；⑦源码断言 `instance.logger` 四级全部转发且保留 `[updater]` 前缀。**字段完整性把 `undefined` 与 `null` 分开断言**：结构化日志序列化会整个丢掉值为 undefined 的键，等于报告少一列，而 null 是“未测得”的既定写法。源码断言先剥注释再匹配，避免文件里讨论 `autoDownload`／差分的说明文字冒充“该设置已生效”；并用反向对照核验断言非空洞——剥注释后 543 → 521 行且代码完好，`disableWebInstaller` 与 `instance.autoDownload = true` 均正确不匹配。**C2 只建立并锁定取证机制，不产出判定结论**：differential/full/unknown 的实际判定按规则在 C4 采集真实日志、C5 写入报告；本步无实机取证，不与 C4 合并。**统一 Mini 门槛已过**：三套 `tsc --noEmit -p`（web/node/根）0 错误、`dotnet build src/runtime/WishfulClaw.sln` 0 警告 0 错误、`test:updater-progress` 35 项、`test:updater-state` 56 项未回归、`test:updater-release-notes` 71 项未回归、`git diff --check` 干净。）
