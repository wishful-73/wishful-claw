...
- [✓] Z3：老大确认完结后由他手动发起收尾：版本 `0.2.28`、合并 main、打 tag、push、Release、progress 记账。
  > **✅ 已发起（2026-09-13）**：老大原话「28 迭代已经都人工复测过了，你帮我把这个需求记录以后就直接进行收尾步骤」——**逐项真机目视复验由老大本人完成**，故本报告 §4 五个 PARTIAL 的"目视缺口"一类在收尾时点记为**已由老大复测确认**（agent 侧仍无屏幕捕获通路，见 §5，两条记账不互相覆盖）。收尾六步的实际执行结果与移交清单见 `docs/progress/v2-iter-28.md`（本文档随收尾提交一起进历史，故此处不写 merge/tag 哈希）。
  > ⚠️ **两点与既有惯例的差异，如实记在此处**：① 本迭代含 R-7（项目级数据目录隐藏 + 临时文档归置）与 R-9（内置服务商懒物化、改的是**存量 provider 记录**的读路径语义），按 v2-iter-27 定的门禁本应**先出生产包、真机安装验证通过后再 merge/tag/push**；本次由老大口头"已人工复测"直接授权收尾，顺序未反转，**装机升级面（生产版读存量数据）仍属未单独验证项**。② R-4.5 配图（`plan.md:969` 未勾）本次仍未做，但"属人类专属动作"的理由已由老大新口径推翻，需求已移交 iter-29 S-19（`docs/plans/iter-v2-29/raw-requirements.md`）。
