...
| [v2-iter-32](progress/v2-iter-32.md) | v0.2.32 | 2026-09-19 | 沙箱模式（工具参数路径边界校验）+ 会话级上下文上限与压缩阈值滑条 + 聊天窗最低宽度保护 + 输入框/审批弹窗/提示条收口 + memory_hot_write 三处修复 |
| [v2-iter-33](progress/v2-iter-33.md) | v0.2.33 | 2026-09-20 | cron 工具对全局会话开放 + Grep 通配符修复 + 记忆系统治理（整理 400 / 召回 / 呈现 / 分页 / 时间筛选 / 多关键词）+ 上下文压缩滚动摘要 + 沙箱放开数据根与项目创建父目录 + 主窗口快捷键与开机静默 + 服务商页收口 |
| [v2-iter-34](progress/v2-iter-34.md) | v0.2.34 | 2026-09-22 | 官网首版（四页面 + 下载区 + 更新日志）+ 全局请求上下文上限 + agent 常驻终端（可起 / 可读 / 可停）+ 更新机制改 6h 巡检与顶栏图标 + 消息分叉 + shell 停止进程生效 + 朗读音色可配 + 流式正文不被工具卡 / 思考块劈开 + 流式渲染降级 + 聊天窗最低宽度 + 消息操作按钮组死链清理 |
