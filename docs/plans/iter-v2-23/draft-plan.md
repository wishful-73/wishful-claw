...
- 已执行：步骤 31 补齐：左侧“对话”分区改为与项目行同款可收起/展开（点击分区行切换全局会话列表，默认展开，子列表缩进/加载更多逻辑与项目子列表一致，chevron 旋转指示）；后按用户反馈移除分区行右侧会话计数；步骤 30 补齐：文件树组件内部图标同步上色（目录图标灰色→琥珀色并按展开切 `Folder`/`FolderOpen`，`EXT_ICONS` 扩充 .cs/.sh/.bat/.sql/.env/.lock/.exe/.zip 等着色覆盖）；TS 三配置 0 错误。
