# v2-iter-23 临时计划（讨论稿）— 会话可靠性与缺陷收口
...
>
> 本文只记录当时的讨论、代码现状和待讨论事项；2026-08-28 用户决定延期正式版，涉及本迭代发布的旧结论均由 `plan.md` 与 `docs/iteration-plan.md` 的新路线取代。

## 一、暂定目标

围绕历史会话恢复、上下文压缩持久化和流式消息可靠性完成当前迭代收口，为后续功能补齐与稳定性验证建立基线。
...
5. 整理集成验证、AOT、安装链和遗留问题证据，作为 v2-iter-25 Release Candidate 准备的输入。
...
### 2.6 正式版延期决策（2026-08-28 更新）

- v2-iter-23 不再承担正式版发布，只完成会话可靠性与缺陷收口。
- v2-iter-24 负责功能补齐与遗留问题推进，v2-iter-25 负责集中修复、完整回归和 Release Candidate 准备。
- v2-iter-26 满足发布门槛并经用户确认后再执行正式版版本迁移、tag、安装包和 Release；具体正式版本号在该迭代规划时确认。
...
### Plan 23-7：当前迭代验证与后续交接

目标：完成本迭代范围验证，整理功能缺口与遗留问题，作为 v2-iter-24/25 的输入；本迭代不执行正式版发布。
...
- 记录当前功能缺口、未关闭问题和人工验收结果；
- 将功能补齐项移入 v2-iter-24；
- 将集中修复、全量回归、AOT/NSIS 和安装观察移入 v2-iter-25；
- 将正式版版本迁移、tag、推送、GitHub Release 和安装包发布移入 v2-iter-26；
- v2-iter-23 仅按普通迭代收口，等待用户确认后再执行合并等动作。
...
- 当前范围的开发构建、回归测试和核心 Agent 流程验证结果可追溯。
- 用户完成本迭代人工验收并明确确认是否完结。
- 功能缺口和剩余问题已分别归入 v2-iter-24/25，正式发布事项归入 v2-iter-26。
...
10. 已决策：v2-iter-25 设置 Release Candidate 阶段，并保留一段安装版人工使用观察。
11. 待 v2-iter-26 规划时确认正式版 Release notes 的定位：自用首个稳定版，还是面向公开用户的产品发布。

## 九、当前状态

- 已完成：知识库、原始迭代规划、v2-iter-22 状态、历史加载、会话恢复、自动压缩和工具结果落库位置的初步核对。
- 已确认：前端最近 5 轮仅用于展示；后端有快照则按快照恢复，无快照则兼容全量恢复；摘要只在手动/自动压缩时产生。
- 路线更新（2026-08-28）：正式版不再绑定 v2-iter-23；v2-iter-24 补功能，v2-iter-25 做集中修复/完整回归/Release Candidate，v2-iter-26 执行正式发布。
...
- 已执行：Plan 23-5 步骤 17 已完成：悬浮块接入聊天区域宽窄调节——`conversationPanelFullWidth` 持久化到 settings-store（version 33 + 迁移守卫，刷新/重启恢复），悬浮块宽窄切换按钮同步驱动 MessageList 与 Composer 列宽（820px ↔ 全宽），右侧面板开关时弹性列自动重适配；同时清理 ui-store 中同名冗余死字段；TS 三配置 0 错误。后续补齐（用户反馈“扩宽聊天区图标不满意”）：宽窄切换按钮图标换成终端面板同款全屏/收起风格（窄态 `Maximize2` / 宽态 `Minimize2`），tooltip 文案不变。Plan 23-5 全部完成。
...
- 已执行：Plan 23-8 步骤 31 已完成：全局对话分区头部加 `MessageSquare` 图标（会话计数后按用户反馈移除，对齐 OpenCowork），zh `sidebar.conversations` “会话”→“对话”；扩展入口 `FolderOpen`→`Plug`、自动化 `CalendarDays`→`Clock3`（均对齐 OpenCowork）；导航链路未动；TS 三配置 0 错误。Plan 23-8 全部完成。
...
- 已执行：Plan 23-8 步骤 32 已完成（用户核验反馈新增需求）：悬浮块新增“查看会话摘要”入口（`ScrollText` 图标，任何会话可见）——`ensureSummaryTab` 单例 summary tab（已存在时刷新 sessionId 绑定），`SessionSummaryPanel` 先扫内存消息、未命中再 `dbLoadMessages` 全量扫 DB 找最新压缩摘要（`isCompactSummaryLikeMessage` 含 legacy 前缀兼容），`messageCount` 订阅驱动原地变异重扫，有摘要时 Markdown 全文渲染 + “已总结前 N 条消息”徽章，无摘要显示“暂无摘要”空态，加载中显示 spinner；zh/en layout.json 新增 `layout.viewSessionSummary` 与 `rightPanel.summary/summaryEmpty/summaryMessages`；tab 标题映射与 `RightPanelHeader` 紫色 `ScrollText` 图标同步接入；TS 三配置 0 错误。
- 已执行：Plan 23-8 步骤 33 已完成（用户核验反馈“多文档预览切换 tab 都显示最后一个文档”）：根因为预览双层 tab 失同步——右侧面板 `preview:*` tab 靠 `previewTabId` 镜像 `previewPanelTabs`，但 `setRightPanelActiveTab` 只更新右侧面板激活 id，`PreviewPanel` 始终按 `activePreviewPanelTabId`（最后打开者）渲染；修复为 `setRightPanelActiveTab` 命中 preview tab 时同步激活底层预览 tab 与 `previewPanelState`，`closeRightPanelTab` 对 preview tab 委托 `closePreviewTab` 双层一致清理；内层 tab 条反向同步既有逻辑不变；TS 三配置 0 错误。
- 已执行：Plan 23-9 步骤 34 已完成（用户要求全量质量排查）：5 组并行子代理初审（IPC 形状失配/静默失败、主进程 36 handler + 外围 + channels、渲染端 43 store + 全部面板、Worker C# + AOT 合规、i18n + 打包）+ 逐项代码级核实（纠正两条子代理误判：sidecar-handlers 实为死代码非故障）+ 镜像/竞态/持久化三项自查；产出 `docs/plans/iter-v2-23/audit-report.md`：高危 8（退出清理三函数零调用、native-worker socket 无 error 监听且不杀子进程、超时分支无限重试、摘要超时 OCE 逃逸、personaId 路径遍历、BashArtifactsCard 两处裸字符串、api:request/skills:market 通道无 handler 待决策）、中危 36（含自查新发现：删会话不清右侧面板绑定 tab）、低危 16 组、存疑 9；Hooks/泄漏/变异/AOT/打包/模块注册六大机械检查全部通过；只查不改，修复待用户确认。
- 已执行：Plan 23-1~23-6 步骤 4-21 代码审查完成（四组并行审查子代理 + 逐项核实）：发现并修复 6 中 4 低问题——①快照失效二元排序确定性 + `db/messages-update` 失效钩子（步骤 4）；②手动压缩 Replace 前重查防 TOCTOU + 机械截断降级不落快照，自动/手动双路径同一守卫（步骤 7/10）；③`dbUpsertMessage` 保留真实错误给 await 调用方 + 日志防未处理拒绝（步骤 19）；④`SessionConversation.InitializeIfEmpty` 原子化消除双 restore 竞态（步骤 21）；⑤`totalTurns` 回传/吸附卡避让悬浮块/压缩状态 i18n 补 chat.json（步骤 12/13/15）；步骤 5/6/8/9/11/14/16/17/18/20 审查通过；其余低危不阻塞项记录在册不修。C# 0 警告 0 错误 + 快照回归 252 断言全过 + TS 三配置 0 错误。
- 已执行：步骤 31 补齐：左侧“对话”分区改为与项目行同款可收起/展开（点击分区行切换全局会话列表，默认展开，子列表缩进/加载更多逻辑与项目子列表一致，chevron 旋转指示）；后按用户反馈移除分区行右侧会话计数，再补对话分区 `MessageSquare` 图标天空蓝（与项目行同色）；步骤 30 补齐：文件树组件内部图标同步上色并对齐 tab 色彩方向（目录图标统一 `sky-500/dark:sky-400` 覆盖树节点/搜索结果/新建输入/头部，`EXT_ICONS` 按状态色惯例改双模式静态类名表，兼容 Tailwind v4 扫描与 6 套主题明暗），二次补齐：`EXT_ICONS` 扩至 120+ 扩展类型并新增 `NAME_ICONS` 全名匹配表（README/Dockerfile/.gitignore/package.json 等），未知文件兜底改蓝色 `File`，三次补齐：清除全部灰色 `text-muted-foreground` 图标统一纳入彩色体系（.md/README 天空蓝、.txt 青、.log/.lock/LICENSE 琥珀，新建文件占位图标改蓝）；TS 三配置 0 错误。
- 已执行：步骤 28 后续补齐（用户反馈“API Key 是必须的”）：新建服务商弹窗 API Key 占位提示去掉“可稍后在配置页补充”，添加按钮与提交校验改为名称 + Key 均必填；二次补齐：模型列表区 `flex-1 min-h-0` 占满剩余高度（去掉固定高度区间与外层整页滚动），滚动只发生在列表内部；三次补齐：模型列表展示排序——已启用模型置顶，组内保持原序（仅展示层）；四次补齐：模型数为 0 时空态显示“请先拉取模型或添加模型” + 两个可点击按钮（复用既有拉取/添加功能）；TS 三配置 0 错误。
- 已执行：Plan 23-9 审计修复实施（用户决策：高中危全修 + 低危静默失败类择修；技能市场保留“内置浏览器固定访问 + 复制安装语句由技能安装小助手执行”路线，平台登录死代码移除；后续用户追加指示处理剩余项）：十批全部完成并本地提交——批1 `1f23f11`（H1/H2/M2–M9/M21 + L1 watch-file/L2 cancelKeys）、批2 `f3fd9d2`（H3/H4/M30–M32，M30 后回滚：用户确认 400 可重试为有意设计）、批3 `7ecacff`（H5/M33–M36）、批4 `282b007`（M10–M20）、批5 `68f16f2`（M22–M29）、批6 `5faa16b`（H6+H7）、批7 `b6546f8`（H8 死代码通道清理，保留 list_installed_skills 与浏览器入口）、批8 `c0adb01`（L1–L4/L7–L11）、批9 `dcd317d`（L12–L16：C# 低危 8 项 + AOT 裸 JsonSerializerOptions 7 处 + 死代码清理 + 打包字段 + hydration 注销）、批10 `5c8e252`（L5/L6 i18n：180 键双语补齐 + 新建 agent/ssh ns + fallbackNS: common + 7 处组件 ns 错配修正，静态校验 MISSING 归零）；每批过 TS 三配置，涉 C# 批次另过 dotnet build 0 警告 0 错误；audit-report.md 已同步修复状态汇总。遗留：审计修复清单已全部闭环（M37 SSH 明文密码回传、M30 HTTP 400 可重试均用户确认为有意设计保留，不修；`repository.ts` 矛盾注释已更正，`IsRetryableStatus` 已加防误改注释）。
- 未执行：用户人工核验步骤 4-31、Plan 23-7（当前迭代全量验证与后续交接，需用户确认）、后续 push、merge；正式版 tag、打包和 Release 已移至 v2-iter-26。
